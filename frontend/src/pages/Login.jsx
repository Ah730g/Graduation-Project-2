...
function Login() {
  ...
  return (
    <div className="flex justify-center items-center flex-1 dark:bg-stone-900">
      <form action="" className="w-80 flex flex-col gap-4" onSubmit={onSubmit}>
        ...
        <input
          ...
          className="w-full px-3 py-5 border dark:border-stone-600 dark:bg-stone-800 dark:text-white outline-none rounded-md"
          ref={refEmail}
        />
        <input
          type="password"
          placeholder={t('auth.password')}
          className="w-full border dark:border-stone-600 dark:bg-stone-800 dark:text-white outline-none rounded-md px-3 py-5"
          ref={refPassword}
        />
        <button
          className="w-full bg-green-600 dark:bg-green-700 text-white px-3 py-5 rounded-md disabled:bg-[#444] dark:disabled:bg-stone-600 disabled:cursor-none"
          disabled={loading}
        >
          {t('auth.login')}
        </button>
        <Link className="underline text-sm text-[#444] dark:text-stone-300 font-bold" to="/signup">
          {t('auth.noAccount')}
        </Link>
      </form>
    </div>
